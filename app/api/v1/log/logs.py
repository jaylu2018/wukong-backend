...
async def delete_log(log_id: int, current_user: User = Depends(get_current_user)):
    ...
    await insert_log(log_type=LogType.AdminLog, log_detail_type=LogDetailType.LogDelete, by_user_id=current_user.id)
...
async def batch_delete_logs(ids: str = Query(..., description="日志ID列表, 用逗号隔开"), current_user: User = Depends(get_current_user)):
    ...
    await insert_log(log_type=LogType.AdminLog, log_detail_type=LogDetailType.LogBatchDelete, by_user_id=current_user.id)

...
async def get_roles(
        ...
        role_code: str = Query(None, description="角色编码"),
        current_user: User = Depends(get_current_user)
):
    q = Q()
    if role_name:
        q &= Q(role_name__contains=role_name)
    if role_code:
        q &= Q(role_code__contains=role_code)

    total, role_objs = await role_service.list(page=current, page_size=size, search=q, order=["id"])
    records = [await role_service.to_dict(role) for role in role_objs]
    data = {"records": records}
    await log_service.create(log_type=LogType.AdminLog, log_detail_type=LogDetailType.RoleGetList, by_user_id=current_user.id)
...
async def get_role(role_id: int, current_user: User = Depends(get_current_user)):
    ...
    await log_service.create(log_type=LogType.AdminLog, log_detail_type=LogDetailType.RoleGetOne, by_user_id=current_user.id)
...
async def create_role(role_in: RoleCreate, current_user: User = Depends(get_current_user)):
    ...
    await log_service.create(log_type=LogType.AdminLog, log_detail_type=LogDetailType.RoleCreateOne, by_user_id=current_user.id)
...
async def update_role(role_id: int, role_in: RoleUpdate, current_user: User = Depends(get_current_user)):
    ...
    await log_service.create(log_type=LogType.AdminLog, log_detail_type=LogDetailType.RoleUpdateOne, by_user_id=current_user.id)
...
async def delete_role(role_id: int, current_user: User = Depends(get_current_user)):
    ...
    await log_service.create(log_type=LogType.AdminLog, log_detail_type=LogDetailType.RoleDeleteOne, by_user_id=current_user.id)
...
async def batch_delete_roles(ids: str = Query(..., description="角色ID列表, 用逗号隔开"), current_user: User = Depends(get_current_user)):
    ...
    await log_service.create(log_type=LogType.AdminLog, log_detail_type=LogDetailType.RoleBatchDeleteOne, by_user_id=current_user.id)
...
async def get_role_menus(role_id: int, current_user: User = Depends(get_current_user)):
    ...
    await log_service.create(log_type=LogType.AdminLog, log_detail_type=LogDetailType.RoleGetMenus, by_user_id=current_user.id)
    return Success(data=data)
...
async def update_role_menus(role_id: int, role_in: RoleUpdateAuthrization, current_user: User = Depends(get_current_user)):
    ...
    await log_service.create(log_type=LogType.AdminLog, log_detail_type=LogDetailType.RoleUpdateMenus, by_user_id=current_user.id)

...
async def get_users(
    ...
    await log_service.create(LogType.AdminLog, LogDetailType.UserGetList, current_user.id)
...
async def get_user(user_id: int, current_user: User = Depends(get_current_user)):
    ...
    await log_service.create(LogType.AdminLog, LogDetailType.UserGetOne, current_user.id)
...
async def create_user(user_in: UserCreate, current_user: User = Depends(get_current_user)):
    ...
    await log_service.create(LogType.AdminLog, LogDetailType.UserCreateOne, current_user.id)
...
async def update_user(user_id: int, user_in: UserUpdate, current_user: User = Depends(get_current_user)):
    ...
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    await log_service.create(LogType.AdminLog, LogDetailType.UserUpdateOne, current_user.id)
...
async def delete_user(user_id: int, current_user: User = Depends(get_current_user)):
    ...
    if not deleted:
        raise HTTPException(status_code=404, detail="User not found")
    await log_service.create(LogType.AdminLog, LogDetailType.UserDeleteOne, current_user.id)
...
async def batch_delete_users(
    ids: str = Query(..., description="用户ID列表, 用逗号隔开"),
    current_user: User = Depends(get_current_user),
):
    ...
    await log_service.create(LogType.AdminLog, LogDetailType.UserBatchDeleteOne, current_user.id)

...
from app.core.config import APP_SETTINGS, TORTOISE_ORM
...
from app.models import Role, Menu, Button, Api, User
from app.models.base import IconType, StatusType, MenuType
...
async def init_users():
    ...
    role_exist = await role_service.model.exists()
    if not role_exist:
        # 超级管理员拥有所有菜单
    ...
    user = await user_service.exists()
    if not user:
        role_super: Role | None = await role_service.get_by_code("R_SUPER")
        user_super: User = await user_service.create(
            UserCreate(
                userName="Soybean",
        ...
        await user_super.roles.add(role_super)
        #
        # role_admin: Role | None = await role_service.get_by_code("R_ADMIN")
        # user_admin = await user_service.create(
        #     UserCreate(
        #         userName="Admin",

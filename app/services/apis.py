...
class ApiService:
    def __init__(self, model: type[Api]):
        self.model = model
    ...
    async def list(
        self, page: int = 1, page_size: int = 10, search: Q = None, order: List[str] = None
    ) -> tuple[int, List[Api]]:
        if search is None:
            search = Q()
        if order is None:

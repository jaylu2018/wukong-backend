...
@pytest.mark.asyncio
async def test_get_constant_routes(async_client: AsyncClient):
    response = await async_client.get("/api/v1/route/constant-routes")
...
async def test_get_user_routes(async_client: AsyncClient, auth_headers):
    response = await async_client.get("/api/v1/route/user-routes", headers=auth_headers)
    assert response.status_code == 200
    json_data = response.json()
...
async def test_route_exists(async_client: AsyncClient, auth_headers):
    route_name = "home"
    response = await async_client.get(f"/api/v1/route/{route_name}/exists", headers=auth_headers)
    assert response.status_code == 200
    json_data = response.json()
...
#     assert response.status_code == 200
# json_data = response.json()
# assert json_data["data"]["deleted_id"] == route_id
